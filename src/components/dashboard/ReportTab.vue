...
import type { DashboardFilters, DashboardReport } from '@/types/dashboard'
...
const barChartSeries = computed(() => {
  if (!props.report?.by_person_building_type?.length)
    return []
...
const stackedBarSeries = computed(() => {
  if (!props.report?.by_person_status?.length)
    return []
