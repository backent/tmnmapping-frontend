...
const itemToDelete = ref<Branch | null>(null)

const items = computed(() => branchStore.items)
const isLoading = computed(() => branchStore.isLoading)
const totalRecords = computed(() => branchStore.pagination.total)
...
const handleEdit = (item: Branch) => {
  router.push({ name: 'branch-edit', params: { id: item.id.toString() } })
}

const confirmDelete = (item: Branch) => {
...
const handleExport = async () => {
  ...
  try {
    await branchStore.exportFile(searchQuery.value.trim() || undefined)
